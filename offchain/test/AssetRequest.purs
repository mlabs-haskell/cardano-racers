...
import Data.Map (toUnfoldable, singleton) as Map
      ...
      \(admin /\ treasury /\ alice) -> do
        ...
        for_ rarities $ \rarity -> do
          withKeyWallet alice do
            assetRequestTokenName <- liftContractM "Could not make required token names" $
              (Value.mkTokenName <=< byteArrayFromAscii) (show rarity)
            assetPrice <- liftContractM "could not get asset price from state" $  AssocMap.lookup rarity (unwrap rs).assetPrices
            let
              depositAddress = scriptHashAddress (unwrap rs).depositScript Nothing
              ...
              assertions =
                ...
                , checkTokenGainAtAddress' (label depositAddress "Deposit")
                    (assetRequestCs /\ assetRequestTokenName /\ BigInt.fromInt 1)
                ]

            runChecks assertions $ lift $
              requestAssetByRarity rp Nothing rarity
  test "User fails to request asset by rarity with incorrect amount paid to operating/treasury" do
    withWallets (walletUtxoDistr /\ walletUtxoDistr /\ walletUtxoDistr)
      \(admin /\ treasury /\ alice) -> do
        let rarity = Rare
        rp <- withKeyWallet admin createRacersParamsHelper
        ...
        withKeyWallet alice do
          ...
          assetRequestTokenName <- liftContractM "Could not make required token names" $
            (Value.mkTokenName <=< byteArrayFromAscii) (show rarity)
          rarePrice <- liftContractM "could not get rare asset price from state" $  AssocMap.lookup rarity (unwrap rs).assetPrices
          ...
          let
            ...
            testIncorrectPayment (treasuryRatio /\ operatingRatio) = do
              ...
              let
                ...
                lockedVal = Value.singleton assetRequestCs assetRequestTokenName $ BigInt.fromInt 1
                ...
                constraints = Constraints.mustReferenceOutput stateTxi
                    ...
                    <> Constraints.mustMintValueWithRedeemer red
                      (Value.singleton assetRequestCs assetRequestTokenName (BigInt.fromInt 1))
                    <> Constraints.mustPayToScript (unwrap rs).depositScript dat DatumInline lockedVal
