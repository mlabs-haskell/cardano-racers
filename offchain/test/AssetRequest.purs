...
suite = group "AssetRequest" do
  test "User mints request token" do
     withWallets (walletUtxoDistr /\ walletUtxoDistr /\ walletUtxoDistr) \(adminKey /\ treasuryKey /\ userKey) -> do
        rp <- withKeyWallet adminKey createRacersParamsHelper
        let carPrices = foldl (flip $ uncurry AssocMap.insert) AssocMap.empty
        ...
        _ <- initRacersStateWithAdminAndTreasury (adminKey /\ treasuryKey) rp carPrices driverPrices
        withKeyWallet userKey $ queryRacersState rp >>= logInfo' <<< show <<< fst
        pure unit
  ...
  initRacersStateWithAdminAndTreasury (admin /\ treasury) rp carPrices driverPrices = do
