module Test.CardanoRacers.Hydra.Main
  ( main
  ) where
...
import Mote (group)
import Test.CardanoRacers.Hydra.Integration (suite) as Integration
...
main = do
  ...
  suite = do
    ...
    group "Cardano Racers Hydra" do
      testTestnetContracts config do
        Integration.suite clusterParamsRef config
